import { ExtensionProperty, PropertyType, Texture, TextureInfo, TextureSampler } from '@gltf-transform/core';
/** Documentation in {@link EXTENSIONS.md}. */
export declare class Clearcoat extends ExtensionProperty {
    readonly propertyType = "Clearcoat";
    readonly parentTypes: PropertyType[];
    readonly extensionName = "KHR_materials_clearcoat";
    static EXTENSION_NAME: string;
    private _clearcoatFactor;
    private _clearcoatRoughnessFactor;
    private _clearcoatNormalScale;
    private clearcoatTexture;
    private clearcoatRoughnessTexture;
    private clearcoatNormalTexture;
    copy(other: this, resolve?: <T extends import("@gltf-transform/core").Property>(t: T) => T): this;
    /**********************************************************************************************
     * Clearcoat.
     */
    /** Clearcoat; linear multiplier. See {@link getClearcoatTexture}. */
    getClearcoatFactor(): number;
    /** Clearcoat; linear multiplier. See {@link getClearcoatTexture}. */
    setClearcoatFactor(clearcoatFactor: number): this;
    /**
     * Clearcoat texture; linear multiplier. The `r` channel of this texture specifies an amount
     * [0-1] of coating over the surface of the material, which may have its own roughness and
     * normal map properties.
     */
    getClearcoatTexture(): Texture;
    /**
     * Settings affecting the material's use of its clearcoat texture. If no texture is attached,
     * {@link TextureInfo} is `null`.
     */
    getClearcoatTextureInfo(): TextureInfo;
    /**
     * Settings affecting the material's use of its clearcoat texture. If no texture is attached,
     * {@link TextureSampler} is `null`.
     */
    getClearcoatTextureSampler(): TextureSampler;
    /** Sets clearcoat texture. See {@link getClearcoatTexture}. */
    setClearcoatTexture(texture: Texture): this;
    /**********************************************************************************************
     * Clearcoat roughness.
     */
    /** Clearcoat roughness; linear multiplier. See {@link getClearcoatRoughnessTexture}. */
    getClearcoatRoughnessFactor(): number;
    /** Clearcoat roughness; linear multiplier. See {@link getClearcoatRoughnessTexture}. */
    setClearcoatRoughnessFactor(clearcoatRoughnessFactor: number): this;
    /**
     * Clearcoat roughness texture; linear multiplier. The `g` channel of this texture specifies
     * roughness, independent of the base layer's roughness.
     */
    getClearcoatRoughnessTexture(): Texture;
    /**
     * Settings affecting the material's use of its clearcoat roughness texture. If no texture is
     * attached, {@link TextureInfo} is `null`.
     */
    getClearcoatRoughnessTextureInfo(): TextureInfo;
    /**
     * Settings affecting the material's use of its clearcoat roughness texture. If no texture is
     * attached, {@link TextureSampler} is `null`.
     */
    getClearcoatRoughnessTextureSampler(): TextureSampler;
    /** Sets clearcoat roughness texture. See {@link getClearcoatRoughnessTexture}. */
    setClearcoatRoughnessTexture(texture: Texture): this;
    /**********************************************************************************************
     * Clearcoat normals.
     */
    /** Clearcoat normal scale. See {@link getClearcoatNormalTexture}. */
    getClearcoatNormalScale(): number;
    /** Clearcoat normal scale. See {@link getClearcoatNormalTexture}. */
    setClearcoatNormalScale(clearcoatNormalScale: number): this;
    /**
     * Clearcoat normal map. Independent of the material base layer normal map.
     */
    getClearcoatNormalTexture(): Texture;
    /**
     * Settings affecting the material's use of its clearcoat normal texture. If no texture is
     * attached, {@link TextureInfo} is `null`.
     */
    getClearcoatNormalTextureInfo(): TextureInfo;
    /**
     * Settings affecting the material's use of its clearcoat normal texture. If no texture is
     * attached, {@link TextureSampler} is `null`.
     */
    getClearcoatNormalTextureSampler(): TextureSampler;
    /** Sets clearcoat normal texture. See {@link getClearcoatNormalTexture}. */
    setClearcoatNormalTexture(texture: Texture): this;
}
