import { ExtensionProperty, PropertyType, Texture, TextureInfo, vec3 } from '@gltf-transform/core';
/** Documentation in {@link EXTENSIONS.md}. */
export declare class Sheen extends ExtensionProperty {
    readonly propertyType = "Sheen";
    readonly parentTypes: PropertyType[];
    readonly extensionName = "KHR_materials_sheen";
    static EXTENSION_NAME: string;
    private _sheenColorFactor;
    private _sheenRoughnessFactor;
    private sheenColorTexture;
    private sheenColorTextureInfo;
    private sheenRoughnessTexture;
    private sheenRoughnessTextureInfo;
    copy(other: this, resolve?: <T extends import("@gltf-transform/core").Property>(t: T) => T): this;
    dispose(): void;
    /**********************************************************************************************
     * Sheen color.
     */
    /** Sheen; linear multiplier. */
    getSheenColorFactor(): vec3;
    /** Sheen; hex color in sRGB colorspace. */
    getSheenColorHex(): number;
    /** Sheen; linear multiplier. */
    setSheenColorFactor(sheenColorFactor: vec3): this;
    /** Sheen; hex color in sRGB colorspace. */
    setSheenColorHex(hex: number): this;
    /**
     * Sheen color texture, in sRGB colorspace.
     */
    getSheenColorTexture(): Texture;
    /**
     * Settings affecting the material's use of its sheen color texture. If no texture is attached,
     * {@link TextureInfo} is `null`.
     */
    getSheenColorTextureInfo(): TextureInfo;
    /** Sets sheen color texture. See {@link getSheenColorTexture}. */
    setSheenColorTexture(texture: Texture): this;
    /**********************************************************************************************
     * Sheen roughness.
     */
    /** Sheen roughness; linear multiplier. See {@link getSheenRoughnessTexture}. */
    getSheenRoughnessFactor(): number;
    /** Sheen roughness; linear multiplier. See {@link getSheenRoughnessTexture}. */
    setSheenRoughnessFactor(sheenRoughnessFactor: number): this;
    /**
     * Sheen roughness texture; linear multiplier. The `a` channel of this texture specifies
     * roughness, independent of the base layer's roughness.
     */
    getSheenRoughnessTexture(): Texture;
    /**
     * Settings affecting the material's use of its sheen roughness texture. If no texture is
     * attached, {@link TextureInfo} is `null`.
     */
    getSheenRoughnessTextureInfo(): TextureInfo;
    /**
     * Sets sheen roughness texture.  The `a` channel of this texture specifies
     * roughness, independent of the base layer's roughness.
     */
    setSheenRoughnessTexture(texture: Texture): this;
}
