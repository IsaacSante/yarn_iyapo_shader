...
import { GLB_BUFFER, PropertyType } from '../constants';
...
export class GLTFReader {
    static read(jsonDoc, options = DEFAULT_OPTIONS) {
        const { json } = jsonDoc;
        ...
        if (assetDef.generator)
            asset.generator = assetDef.generator;
        if (assetDef.minVersion)
            asset.minVersion = assetDef.minVersion;
        /** Extensions (1/2). */
        const extensionsUsed = json.extensionsUsed || [];
        const extensionsRequired = json.extensionsRequired || [];
        for (const Extension of options.extensions) {
            if (extensionsUsed.includes(Extension.EXTENSION_NAME)) {
                const extension = doc.createExtension(Extension)
        ...
        context.nodes = nodeDefs.map((nodeDef) => {
            ...
            if (nodeDef.matrix !== undefined) {
                node.setTranslation(getTranslation([], nodeDef.matrix));
                node.setRotation(getRotation([], nodeDef.matrix));
                node.setScale(getScaling([], nodeDef.matrix));
            }
        ...
        context.animations = animationDefs.map((animationDef) => {
            ...
            const samplers = samplerDefs.map((samplerDef) => {
                const sampler = doc.createAnimationSampler()
                    ...
                    .setInterpolation(samplerDef.interpolation || "LINEAR" /* LINEAR */);
 ...
 * @hidden
 */
function getInterleavedArray(accessorDef, jsonDoc) {
 ...
 * @hidden
 */
function getAccessorArray(accessorDef, jsonDoc) {
    const bufferViewDef = jsonDoc.json.bufferViews[accessorDef.bufferView];
    const bufferDef = jsonDoc.json.buffers[bufferViewDef.buffer];
    const resource = bufferDef.uri ? jsonDoc.resources[bufferDef.uri] : jsonDoc.resources[GLB_BUFFER];
    const TypedArray = ComponentTypeToTypedArray[accessorDef.componentType];
 ...
 * @hidden
 */
function getSparseArray(accessorDef, jsonDoc) {
    const TypedArray = ComponentTypeToTypedArray[accessorDef.componentType];
    const elementSize = Accessor.getElementSize(accessorDef.type);
    let array;
    ...
    const indicesDef = Object.assign(Object.assign(Object.assign({}, accessorDef), accessorDef.sparse.indices), { count, type: 'SCALAR' });
    const valuesDef = Object.assign(Object.assign(Object.assign({}, accessorDef), accessorDef.sparse.values), { count });
    const indices = getAccessorArray(indicesDef, jsonDoc);
